...
HEXAGON.prototype = {
	...
	render: function (context) {
		...
		context.closePath();
		context.fillStyle = 'hsla(210, 80%, ' + this.LUMINANCE.MIN + '%, 0.3)';
		context.fill();
...
var head_ele = document.querySelector('.header');
var head_ele_height = head_ele.offsetHeight;
// var head_ele_width = head_ele.offsetWidth;
// console.log(head_ele_height);
var footer_ele = document.querySelector('.footer');
var footer_ele_height = parseInt(footer_ele.offsetHeight);
// var footer_ele_width = footer_ele.offsetWidth;
// console.log(footer_ele_height);
var search_ele = document.querySelector('.searchb_form');
var search_ele_height = parseInt(search_ele.offsetHeight);
// var search_ele_width = search_ele.offsetWidth;
// console.log(search_ele_height);

var not_found = document.querySelector('.not_found');
// min-height: calc(100% - );
if (not_found)
	not_found.setAttribute("style", `min-height: calc(100% - ${search_ele_height}px - ${head_ele_height}px - ${footer_ele_height}px);`);

var search_b = document.querySelector('.searchb_form');
var search_b_val = document.querySelector('.searchb');
if (search_b) {
	search_b.addEventListener('submit', async e => {
		e.preventDefault();
		const searchFor = search_b_val.value.split(' ').join('+');
		window.location = `/find?q=${searchFor}`;
	});
};
if (document.querySelectorAll('.result_card').length === 1) {
	document.querySelector('.result_row').setAttribute("style", `margin-bottom:2.8rem`);
}